...
public class RootController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        sliderPlay.valueProperty().addListener(new ChangeListener<Number>(){
            @Override
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                double slideMinValue = newValue.doubleValue() - oldValue.doubleValue();                
                double slidePoint = newValue.doubleValue() / 100 * mediaPlayer.getTotalDuration().toSeconds();
                if( Math.abs(slideMinValue)>0.5 ){
                    mediaPlayer.seek(Duration.seconds(slidePoint));
                }
            }
        });
